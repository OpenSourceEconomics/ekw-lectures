...
class Task(evil):
    ...
    def exec_command(self, cmd, **kw):
        ...
        if self.env.PATH:
            ...
            env["PATH"] = (
                self.env.PATH if isinstance(self.env.PATH, str) else os.pathsep.join(self.env.PATH)
            )
    ...
    def display(self):
        ...
        if self.generator.bld.progress_bar == 2:
            ...
            try:
                ...
                outs = ""
            return (
                f"|Total {master.total}|Current {cur()}|Inputs {ins}|Outputs {outs}|Time {ela}|\n"
            )
    ...
    def runnable_status(self):
        ...
        try:
            ...
            Logs.debug(
                "task: task %r must run: it was never run before or the task code changed", self
            )
            return RUN_ME
        ...
        for node in self.outputs:
            ...
            if sig != key:
                Logs.debug(
                    "task: task %r must run: an output node was produced by another task", self
                )
                return RUN_ME
            if not node.exists():
...
reg_act = re.compile(
    r"(?P<backslash>\\)|(?P<dollar>\$\$)|(?P<subst>\$\{(?P<var>\w+)(?P<code>.*?)\})", re.M
)
