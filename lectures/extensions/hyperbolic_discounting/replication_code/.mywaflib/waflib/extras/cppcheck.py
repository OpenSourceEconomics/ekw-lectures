...
class cppcheck(Task.Task):
        ...
        for div in page.findall("div"):
            ...
            if div.get("id") == "content":
                ...
                formatter = CppcheckHtmlFormatter(
                    linenos=True, style="colorful", hl_lines=hl_lines, lineanchors="line"
                )
    ...
    def _create_html_table(self, content, files):
        ...
        for name, val in files.items():
            ...
            errors = sorted(
                val["errors"], key=lambda e: int(e["line"]) if "line" in e else sys.maxint
            )
            for e in errors:
                if not "line" in e:
                    ...
                    if e["severity"] == "error":
                        attr = 'class="error"'
                    s = '<tr><td><a href="{}#line-{}">{}</a></td>'.format(f, e["line"], e["line"])
...
CPPCHECK_HTML_ERROR = '<span style="background: #ffaaaa;padding: 3px;">&lt;--- %s</span>\n'
