...
def find_ticc(conf):
    conf.find_program(
        ["cl6x"], var="CC", path_list=opj(getattr(Options.options, "ti-cgt-dir", ""), "bin")
    )
...
def find_tild(conf):
    conf.find_program(
        ["lnk6x"], var="LINK_CC", path_list=opj(getattr(Options.options, "ti-cgt-dir", ""), "bin")
    )
...
def find_tiar(conf):
    conf.find_program(
        ["ar6x"], var="AR", path_list=opj(getattr(Options.options, "ti-cgt-dir", ""), "bin")
    )
