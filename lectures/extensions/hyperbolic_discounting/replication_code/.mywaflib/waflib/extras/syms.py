...
class gen_sym(Task):
    def run(self):
        ...
        syms = [
            m.group("symbol")
            for m in re_nm.finditer(self.generator.bld.cmd_and_log(cmd, quiet=STDOUT, **kw))
        ]
...
@feature("syms")
@after_method(
    "process_source", "process_use", "apply_link", "process_uselib_local", "propagate_uselib_vars"
)
def do_the_symbol_stuff(self):
    ...
    if compiled_tasks:
        ...
        self.gen_sym_tasks = [
            self.create_task("gen_sym", x, x.change_ext(".%d.sym" % self.idx)) for x in ins
        ]
