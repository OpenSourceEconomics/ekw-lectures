...
def make_buildcopy(self):
    ...
    nodes = [
        to_src_nodes(n) for n in getattr(self, "buildcopy_source", getattr(self, "source", []))
    ]
    node_pairs = [(n, n.get_bld()) for n in nodes]
    self.create_task(
        "buildcopy", [n[0] for n in node_pairs], [n[1] for n in node_pairs], node_pairs=node_pairs
    )
