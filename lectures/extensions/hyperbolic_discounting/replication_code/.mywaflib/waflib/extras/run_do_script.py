...
if sys.platform == "darwin":
    ...
    STATA_PATHS = [f"/Applications/Stata/{sv}.app/Contents/MacOS/{sv}" for sv in STATA_NAMES]
...
class run_do_script_base(Task.Task):
    ...
    run_str = (
        '${PREPEND} "${STATACMD}" ${STATAFLAGS} "${SRC[0].abspath()}" "${DOFILETRUNK}" ${APPEND}'
    )
    shell = True
...
def apply_run_do_script(tg):
    ...
    Logs.debug(f"deps: found dependencies {tsk.dep_nodes!r} for running {src_node.relpath()!r}")
