...
class eclipse(Build.BuildContext):
    ...
    def create_cproject(self, appname, workspace_includes=[], pythonpath=[]):
        ...
        if hasc:
            project = self.impl_create_cproject(
                sys.executable, waf_executable, appname, workspace_includes, cpppath, source_dirs
            )
    ...
    def impl_create_project(self, executable, appname, hasc, hasjava, haspython, waf_executable):
        ...
        if hasc:
            ...
            self.add(
                ...
                launchConfiguration,
                "stringAttribute",
                {"key": "org.eclipse.ui.externaltools.ATTR_LOCATION", "value": waf_executable},
            )
            self.add(
                doc,
                launchConfiguration,
            ...
            self.add(
                ...
                launchConfiguration,
                "stringAttribute",
                {"key": "org.eclipse.ui.externaltools.ATTR_TOOL_ARGUMENTS", "value": "build"},
            )
            self.add(
                doc,
                launchConfiguration,
    ...
    def impl_create_cproject(
        self, executable, waf_executable, appname, workspace_includes, cpppath, source_dirs=[]
    ):
        doc = Document()
        ...
        storageModule = self.add(
            doc, cconf, "storageModule", {"moduleId": "cdtBuildSystem", "version": "4.0.0"}
        )
        config = self.add(
            doc,
            storageModule,
        ...
        folderInfo = self.add(
            doc, config, "folderInfo", {"id": cconf_id + ".", "name": "/", "resourcePath": ""}
        )
        ...
        for tool_name in ("Assembly", "GNU C++", "GNU C"):
            ...
            if cpppath or workspace_includes:
                ...
                for i in cpppath:
                    self.add(
                        doc, option, "listOptionValue", {"builtIn": "false", "value": '"%s"' % (i)}
                    )
        ...
        storageModule = self.add(
            doc, cproject, "storageModule", {"moduleId": "cdtBuildSystem", "version": "4.0.0"}
        )
