...
class PdflatexBuilder:
    ...
    def create_task(self, task_gen, src, tgt):  # pylint: disable=no-self-use
        ...
        latex_task.texinputs_nodes = _make_texinputs_nodes(task_gen, [orig_tex_node.parent])
...
def _node_or_bust(node_or_path, node_func):
    return node_or_path if isinstance(node_or_path, waflib.Node.Node) else node_func(node_or_path)
...
def apply_sphinx(task_gen):
    ...
    try:
        ...
        raise waflib.Errors.WafError("Sphinx task generator missing necessary keyword: builders")

    # Check for dupes.
    if len(requested_builders) != len(set(requested_builders)):
        raise waflib.Errors.WafError("Sphinx 'builder' keyword cannot contain duplicates.")
