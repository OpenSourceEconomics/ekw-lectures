...
def check_waf_version(self, mini="1.9.99", maxi="2.1.0", **kw):
    ...
    """
    self.start_msg("Checking for waf version in {}-{}".format(str(mini), str(maxi)), **kw)
    ver = Context.HEXVERSION
    if Utils.num2ver(mini) > ver:
        self.fatal(
            "waf version should be at least {!r} ({!r} found)".format(Utils.num2ver(mini), ver)
        )
    if Utils.num2ver(maxi) < ver:
        self.fatal(
            "waf version should be at most {!r} ({!r} found)".format(Utils.num2ver(maxi), ver)
        )
    self.end_msg("ok", **kw)
...
def find_program(self, filename, **kw):
    ...
    if not kw.get("quiet"):
        self.to_log(f"find program={filename!r} paths={path_list!r} var={var!r} -> {ret!r}")
