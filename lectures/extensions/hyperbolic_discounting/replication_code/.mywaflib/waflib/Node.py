    ...
    * The methods bound to a :py:class:`waflib.Build.BuildContext` (require ``bld.srcnode`` and ``bld.bldnode``)
    ...
    def __init__(self, name, parent):
        ...
        if parent:
            if name in parent.children:
                raise Errors.WafError(f"node {name} exists in the parent files {parent!r} already")
    ...
    def write_json(self, data, pretty=True):
        ...
        if not pretty:
            ...
            newline = ""
        output = (
            json.dumps(data, indent=indent, separators=separators, sort_keys=sort_keys) + newline
        )
        ...
        * ``..`` find files having two dot characters
        ...
        if kw.get("generator"):
            return Utils.lazy_generator(
                self.ant_iter, (ant_sub_matcher, maxdepth, pats, dir, src, remove, quiet)
            )
