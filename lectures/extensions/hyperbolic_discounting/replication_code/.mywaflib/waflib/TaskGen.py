...
class task_gen:
    ...
    * The attribute 'idx' is a counter of task generators in the same path
    ...
    def get_hook(self, node):
        ...
        raise Errors.WafError(f"File {node!r} has no mapping in {keys!r} (load a waf tool?)")
...
def process_rule(self):
    ...
    * deep_inputs: whether the task must depend on the input file tasks too (False by default)
    ...
    name = str(
        getattr(self, "name", None) or self.target or getattr(self.rule, "__name__", self.rule)
    )
    ...
    key = Utils.h_list(
        (name, self.rule, chmod, shell, color, cls_str, cls_keyword, scan_val, _vars, deep_inputs)
    )
