...
def options(opt):
    opt.add_option(
        "--msvc_version", type="string", help='msvc version, eg: "msvc 10.0,msvc 9.0"', default=""
    )
...
""".format(
    ...
    try:
        ...
        Logs.debug(
            "msvc: get_msvc_version: %r %r %r -> failure %s", compiler, version, target, str(e)
        )
        conf.fatal(
...
def gather_msvc_targets(conf, versions, version, vc_path):
    ...
    if os.path.isfile(os.path.join(vc_path, "VC", "Auxiliary", "Build", "vcvarsall.bat")):
        ...
        targets["x86"] = target_compiler(
            conf, "msvc", "x86", version, "", os.path.join(vc_path, "Bin", "vcvars32.bat")
        )
    if targets:
