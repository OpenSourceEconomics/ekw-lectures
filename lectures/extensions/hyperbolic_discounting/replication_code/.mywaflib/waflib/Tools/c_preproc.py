    ...
    def start(self, node, env):
        ...
        while self.lines:
            ...
            try:
                ...
                if Logs.verbose:
                    Logs.debug(
                        "preproc: line parsing failed (%s): %s %s", e, line, traceback.format_exc()
                    )
...
def scan(task):
    ...
    try:
        ...
        raise Errors.WafError(
            '%r is missing a feature such as "c", "cxx" or "includes": ' % task.generator
        )
    ...
    if go_absolute:
        ...
        nodepaths = [
            x for x in incn if x.is_child_of(x.ctx.srcnode) or x.is_child_of(x.ctx.bldnode)
        ]
