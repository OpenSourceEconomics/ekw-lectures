    ...
    if not dupe:
        for (k, v) in uids.items():
            if len(v) > 1:
                ...
                for tsk in v:
                    Logs.error(
                        "  - object %r (%r) defined in %r", tsk.__class__.__name__, tsk, tg_details
                    )
        ...
        for x in ("before", "after"):
            for y in Utils.to_list(getattr(cls, x, [])):
                if not Task.classes.get(y):
                    Logs.error(
                        "Erroneous order constraint %r=%r on task class %r", x, y, cls.__name__
                    )
        if getattr(cls, "rule", None):
            Logs.error(
                'Erroneous attribute "rule" on task class %r (rename to "run_str")', cls.__name__
            )
...
def enhance_lib():
    ...
    def ant_glob(self, *k, **kw):
        if k:
            ...
            for pat in lst:
                ...
                if "." in sp:
                    Logs.error(
                        "In ant_glob pattern %r: '.' means 'one dot', not 'current directory'", k[0]
                    )
    ...
    def ant_iter(
        self, accept=None, maxdepth=25, pats=[], dir=False, src=True, remove=True, quiet=False
    ):
        if remove:
            try:
    ...
    def check_err_order(self):
        if not hasattr(self, "rule") and not "subst" in Utils.to_list(self.features):
            for x in ("before", "after", "ext_in", "ext_out"):
                if hasattr(self, x):
                    Logs.warn(
                        "Erroneous order constraint %r on non-rule based task generator %r", x, self
                    )
        else:
            for x in ("before", "after"):
                for y in self.to_list(getattr(self, x, [])):
                    if not Task.classes.get(y):
                        Logs.error(
                            "Erroneous order constraint %s=%r on %r (no such class)", x, y, self
                        )
