    ...
    if self.install_path:
        if self.install_from:
            pyd = Utils.subst_vars(
                "{}/{}".format(self.install_path, node.path_from(self.install_from)), self.env
            )
        else:
            pyd = Utils.subst_vars(
...
def check_python_version(conf, minver=None):
    ...
    assert len(lines) == 5, "found {!r} lines, expected 5: {!r}".format(len(lines), lines)
...
def options(opt):
    ...
    pyopt.add_option(
        "--python", dest="python", help="python binary to be used [Default: %s]" % sys.executable
    )
    pyopt.add_option(
        "--pythondir",
        dest="pythondir",
