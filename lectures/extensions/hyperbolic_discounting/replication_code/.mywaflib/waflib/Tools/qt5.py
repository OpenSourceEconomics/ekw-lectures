...
class qm2rcc(Task.Task):
    ...
    def run(self):
        ...
        txt = "\n".join(
            ["<file>%s</file>" % k.path_from(self.outputs[0].parent) for k in self.inputs]
        )
...
def find_single_qt5_lib(self, name, uselib, qtlibs, qtincludes, force_static):
    ...
    for tup in lib_names():
        ...
        if os.path.exists(path):
            ...
            env.append_unique(
                "INCLUDES_" + uselib, os.path.join(qtincludes, name.replace("Qt5", "Qt"))
            )
            return k
    return False
    ...
    try:
        ...
        for i in self.qt5_vars:
            ...
            if Utils.unversioned_sys_platform() == "darwin":
                ...
                env.append_unique(
                    "INCLUDES_" + uselib, os.path.join(env.QTLIBS, frameworkName, "Headers")
                )
            else:
