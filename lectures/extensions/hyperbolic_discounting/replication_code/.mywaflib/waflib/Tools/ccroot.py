...
USELIB_VARS["c"] = {"INCLUDES", "FRAMEWORKPATH", "DEFINES", "CPPFLAGS", "CCDEPS", "CFLAGS", "ARCH"}
USELIB_VARS["cxx"] = {
    "INCLUDES",
    "FRAMEWORKPATH",
    "DEFINES",
...
USELIB_VARS["dprogram"] = {"LIB", "STLIB", "LIBPATH", "STLIBPATH", "LINKFLAGS", "RPATH", "LINKDEPS"}
USELIB_VARS["dshlib"] = {"LIB", "STLIB", "LIBPATH", "STLIBPATH", "LINKFLAGS", "RPATH", "LINKDEPS"}
    ...
    * ``libfoo.so.14 → libfoo.so.14.15.16``
    ...
    if not re_vnum.match(self.vnum):
        raise Errors.WafError(
            "Invalid vnum {!r} for target {!r}".format(self.vnum, getattr(self, "name", self))
        )
    ...
    if getattr(self, "install_task", None):
        ...
        if self.env.DEST_OS == "openbsd":
            ...
            t1 = self.add_install_as(
                install_to=f"{path}/{libname}", install_from=node, chmod=self.link_task.chmod
            )
            self.vnum_install_task = (t1,)
        else:
            t1 = self.add_install_as(
                install_to=path + os.sep + name3, install_from=node, chmod=self.link_task.chmod
            )
