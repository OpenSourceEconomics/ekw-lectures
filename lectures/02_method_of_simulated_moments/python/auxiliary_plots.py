...
def plot_criterion_params(params, param_names, criterion_msm, radius): 
    ...
    lbounds = [params.loc[name, "value"] - dev for name, dev in zip(param_names, 
                                                                    deviations)]
    ubounds = [params.loc[name, "value"] + dev for name, dev in zip(param_names, 
                                                                    deviations)]
    detail = 20         
...
def plot_chatter_numagents_both(
    seeds, 
    num_agents, 
    calc_moments, 
    replace_nans, 
    ...
    for num in num_agents:
        ...
        moments_true = {"Choice Frequencies": replace_nans(
